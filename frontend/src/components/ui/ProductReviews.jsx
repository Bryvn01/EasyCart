...
const ProductReviews = ({ rating = 4.8, reviewCount = 204, recentReview = "Fast delivery, product was genuine!" }) => {
  const fullStars = Math.floor(rating);
  const hasHalfStar = rating % 1 >= 0.5;

  return (
    <div className="product-reviews" style={{ marginBottom: '12px' }}>
      {/* Star Rating */}
      ...
      {recentReview && (
        <div className="flex items-start gap-2 text-xs text-gray-600 bg-gray-50 p-2 rounded">
          ...
          <span className="line-clamp-2 italic">"{recentReview}"</span>
        </div>
      )}
    </div>
  );
