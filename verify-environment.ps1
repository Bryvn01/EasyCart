...
if (Test-Path "$rootDir\backend\.env") {
    ...
    $hasSecretKey = ($backendContent -match "(?m)^SECRET_KEY=.+$") -and ($backendContent -notmatch "(?m)^SECRET_KEY=<your_django_secret_key>")
    if ($hasSecretKey) {
...
if (Test-Path "$rootDir\frontend\.env") {
    ...
    if ($frontendContent -match "(?m)^REACT_APP_CLOUDINARY_CLOUD_NAME=.+$") {
...
if (Test-Path "$rootDir\admin-dashboard\.env") {
    ...
    if ($adminContent -match "(?m)^REACT_APP_CLOUDINARY_CLOUD_NAME=.+$") {
        Write-Host "   ✅ CLOUDINARY_CLOUD_NAME is configured" -ForegroundColor Green
    } else {
        Write-Host "   ⚠️  CLOUDINARY_CLOUD_NAME not set" -ForegroundColor Red
    }
