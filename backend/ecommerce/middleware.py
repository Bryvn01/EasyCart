...
class ErrorHandlingMiddleware:
    ...
    def process_exception(self, request, exception):
        ...
        if isinstance(exception, PermissionDenied):
            return JsonResponse(
                {"error": "Permission denied", "message": "You do not have permission to perform this action"},
                status=403,
            )
...
def custom_exception_handler(exc, context):
    ...
    if response is not None:
        custom_response_data = {"error": True, "message": "An error occurred", "details": response.data}
