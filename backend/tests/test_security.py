...
class SecurityTestCase(TestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name="Test Product", description="Test Description", price=10.00, category=self.category, stock=100
        )

    def test_xss_protection_in_product_name(self):
        """Test that product names are properly escaped"""
        malicious_name = '<script>alert("xss")</script>'
        product = Product.objects.create(name=malicious_name, description="Test", price=10.00, category=self.category)
    ...
    def test_price_validation_nan_injection(self):
        ...
        response = self.client.get("/api/products/", {"price_min": "NaN", "price_max": "Infinity"})

        # If API endpoint doesn't exist, try regular endpoint
        if response.status_code == 404:
            response = self.client.get("/products/", {"price_min": "NaN", "price_max": "Infinity"})
    ...
    def test_authentication_required_for_sensitive_operations(self):
        ...
        response = self.client.post("/api/products/", {"name": "Unauthorized Product", "price": 100.00})

        # If API endpoint doesn't exist, try regular endpoint
        if response.status_code == 404:
            response = self.client.post("/products/", {"name": "Unauthorized Product", "price": 100.00})
