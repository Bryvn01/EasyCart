...
class MPesaGateway:
    ...
    def get_access_token() -> str:
        ...
        try:
            ...
            PaymentLog.objects.create(payment=None, event="mpesa_oauth_error", message=f"Status: {response.status_code}")
            raise Exception("Failed to obtain M-Pesa access token")
        except requests.exceptions.RequestException as e:
            PaymentLog.objects.create(payment=None, event="mpesa_oauth_error", message="Network error")
    ...
    def initiate_stk_push(payment: Payment, phone_number: str) -> dict:
        ...
        try:
            response = requests.post(MPesaGateway.STK_PUSH_URL, json=payload, headers=headers, timeout=30)
            # Log without sensitive data
            PaymentLog.objects.create(payment=payment, event="mpesa_stk_push", message=f"Status: {response.status_code}")
            if response.status_code == 200:
                return response.json()
            else:
                raise Exception(f"STK Push failed with status {response.status_code}")
        except requests.exceptions.RequestException as e:
            PaymentLog.objects.create(payment=payment, event="mpesa_stk_push_error", message="Network error")
            raise Exception("M-Pesa service unavailable") from e
        ...
        try:
            ...
            if not payment:
                PaymentLog.objects.create(
                    payment=None, event="mpesa_callback_error", message="Payment not found"
                )
                return False
            ...
            payment.raw_response = {"result_code": result_code, "timestamp": datetime.datetime.now().isoformat()}
            payment.save()
            PaymentLog.objects.create(payment=payment, event="mpesa_callback", message=f"Result: {result_code}")
            return True
        except Exception as e:
            logger.error(f"M-Pesa callback error: {str(e)}")
            PaymentLog.objects.create(payment=None, event="mpesa_callback_exception", message="Callback processing failed")
            return False
