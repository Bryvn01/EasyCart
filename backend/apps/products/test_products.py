...
class ReviewAPITests(APITestCase):
    ...
    def test_create_review_authenticated(self):
        ...
        self.client.force_authenticate(user=self.user)
        url = reverse("review-create", kwargs={"product_id": self.product.id})
...
class WishlistTests(APITestCase):
    ...
    def test_remove_from_wishlist(self):
        ...
        url = reverse("wishlist-remove", kwargs={"pk": wishlist_item.id})
        response = self.client.delete(url)
        self.assertIn(
            response.status_code,
...
class ProductStockTests(APITestCase):
    ...
    def test_check_stock_availability(self):
        ...
        url = reverse("product-stock", kwargs={"pk": self.product.id})
        response = self.client.get(url)
        self.assertIn(
            response.status_code, [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]
        )
